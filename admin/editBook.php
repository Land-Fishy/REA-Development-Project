<?php
	...
	$error = '';
	try{
		$db = new PDO('mysql:host=localhost;dbname=lewiswolfe;charset=utf8',$user, $pass);
		...
		if(isset($_GET['book'])){
			...
			header("location: http://localhost/rea-development-project/");
		}
		$genre = getGenre($db);
		...
		if(isset($_POST['upload']) && !empty($_FILES['image'])){
			...
			if(substr($bestand['type'], 0, 5) == 'image'){
				...
				$stmt = $db->prepare("UPDATE books (image, title, release_date, description, amazon_code, pagecount, genre, subgenre) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
<html>
	...
	<body>
		<form method="post" enctype="multipart/form-data">
			...
			<input type="text" name="asin" id="asin" value="<?= $book['amazon_code'];?>">
